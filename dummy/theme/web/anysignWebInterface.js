...
document.write("<script language=javascript src=\"libs/jquery-2.1.4.min.js\" charset=\"utf-8\"></script>");
...
var CALLBACK_TYPE_STOP_RECORDING = 15;
	...
	*/
     ...
     * @returns {*} 是否设置成功
     */
    this.setOrgData = function(template_type, orgData)
    {
        if(core)
            return core._setOrgData(template_type, orgData);
        else
            return false;
    }
    ...
    /**
     * 一次业务完成(签名、拍照均做完)后，调用此接口返回签名状态位参数
     * @return 0 1 2
     */
    this.getSignatureStatus = function()
    {
        return core._getSignatureStatus();
    }

    /**
     * 一次业务完成(签名、拍照均做完)后，调用此接口返回批注状态位参数
     * @return 0 1 2
     */
    this.getCommentStatus = function()
    {
        return core._getCommentStatus();
    }
     ...
     * @return 加密的业务数据。
     */
    this.getUploadDataGramForText = function()
    {
        return core._getUploadDataGramForText();
    }
...
//  }
	...
	{
		return "AnySign_V1.3.3_Web_2.4.6";
	}
...
function SignatureConfig(signer, signRule)
{
    ...
    this.signature_stroke_width = 7.5;//签名笔划粗细
    this.signature_max_times = 3;//签名手写识别错误上限
    this.signatureImgRatio = 1.0;//签名图片清晰度
...
function CommentConfig(signer, signRule)
{
    ...
    this.comment_stroke_width = 7.5;//批注笔划粗细
    this.nessesary = false;//是否为必签项
    this.isdistinguish = false;//是否开启识别
    this.comment_max_times = 3;//批注手写识别错误上限
    this.commentImgRatio = 1.0;//批注图片清晰度
...
var Signer = function(name, id_num, Signer_Type)
{
...
var DataFormat =
{
    ...
    ANYSIGN_SCRIPT : "anysign/script",
    SCRIPT_PNG : "script/png",
    REMARK_PNG : "remark/png",
...
var BioType = 
{
    ...
    /** 签署动作视频 **/
    VIDEO_SIGNER_ACTION : 2,
    /** 其他视频 **/
    VIDEO_SIGNER_OTHER : 3,
    /** 签名人复述录音 **/
    SOUND_SIGNER_RETELL : 4,
    /** 签名人自定义录音 **/
    SOUND_SIGNER_OTHER : 5,
    /** 签名人当前位置 **/
    SIGNER_POSITION : 6,
    /** 签名人面部正面照 **/
    PHOTO_SIGNER_FACE : 1000,
    /** 签名人其他证件照 **/
    PHOTO_SIGNER_CARD : 1020,
    /** 签名动作照片 **/
    PHOTO_SIGNER_ACTION : 1040,
    /** 签名现场环境照片 **/
    PHOTO_SIGNER_ENVIRONMENT : 1060,
    /** 其他类型的照片 **/
    PHOTO_SIGNER_OTHER : 1600,
    /** 其他类型的证据，包括字符串和文本 **/
    SIGNER_OTHER : 5000
};
